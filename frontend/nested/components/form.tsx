...
import {find, join, kebabCase, map, max, sampleSize, size} from "lodash";
...
export const MyInput = ({label, type = "text", desc, err, placeholder = "", id = kebabCase(label), ...rest}: InputParams & InputHTMLAttributes) => {
    return (<div className={cx("flex flex-row py-2 px-3", {"tooltip tooltip-error tooltip-open": !!err})} data-tip={err}>
            <label for={id}>
                {label}
                <p className="text-xs text-base-content/50">{desc}</p>
            </label>
            <input id={id} type={type} className="outline-none text-right grow h-5" placeholder={placeholder} {...rest}/>
        </div>)
}

type SelectParams = { label: string, desc?: string, id?: string, options: Option[] }
export const MySelect = ({label, id = kebabCase(label), desc, options, ...rest}: SelectParams & SelectHTMLAttributes) => {
    ...
    return (<div className="flex flex-row py-2 px-3">
            ...
            <select id={id} className="outline-none text-right h-5" value={dftValue?.value} {...rest}>
