...
import {find, first, get, last, map} from "lodash";
...
export const LogsPage = ({params}: RoutePropsForPath<"/:id/*">) => {
    ...
    return (
        <Group orientation="vertical">
            <Panel>
                <div className="size-full text-xs overflow-auto bg-neutral text-neutral-content" ref={scrollRef} onScroll={onScroll}>
                    {map(logs, (l, i) => (
                        <pre data-prefix={i} className={cx("hover:bg-neutral-content/35", {"bg-neutral-content/35": text == l})} onClick={() => setSelectText(l)}><code>{l}</code></pre>
                    ))}
                </div>
            </Panel>
            <Separator className="h-2 w-full bg-base-content/10 hover:bg-base-content/20 cursor-row-resize focus:outline-none"
                       aria-orientation="vertical"/>
            <Panel defaultSize={120} className="bg-amber-100 text-gray-600 text-sm">
                {text}
            </Panel>
        </Group>
