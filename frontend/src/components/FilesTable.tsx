...
const FileRow = ({data, depth = 0, perm = false}: {data: FileNode | null, depth?: number, perm?: boolean}) => {
    if (!data) return ""

    const {n: name, p: path, s: size, m, mod, dir, diff, child} = data
    const rwx = map("rwxrwxrwx", (c, i) => ((m&(1<<(9-1-i))) != 0) ? c : "-")
    const executable = rwx[2] == "x"
    const fileType = (m&FileMode.ModeDir) != 0 ? "Folder" : (m&FileMode.ModeSymlink) != 0 ? "Symlink" : "File"
    const fileColor = {
    ...
    return (
        <>
            ...
            {open && dir && map(child, (f, i) => <FileRow key={`[${i}]/${f?.p}/${f?.n}`} data={f} depth={depth + 1} perm={perm} />)}
        </>
    )
}

const FileIcon = ({open, executable, fileType, className}: {open: boolean, executable: boolean, fileType: "Folder" | "Symlink" | "File", className: string}) => {
    return (
        <div>
            {
                fileType == "Folder" ?
