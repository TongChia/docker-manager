...
export const formatSize = (v?: number, n: number = 0, units: string[] = ["B", "KB", "MB", "GB", "TB"]): string => v ? (v > 1000 && n < units.length) ? formatSize(v / 1000, n + 1) : `${v.toFixed(n-1)} ${units[n]}` : 'Zero KB'
